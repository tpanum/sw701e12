...
It has a \verb+host+ and a \verb+port+ as parameters and the first part of the pipeline can be seen in Listing~\ref{lst:tcpclientsrc}.
...
One pipeline which reads the drones' video feed and serves as a server, and a client pipeline \deno{CP} for displaying the video feed.
...
The server pipeline \deno{SP} reads the drones' video feed and multicast it via Real-time Transport Protocol(RTP).
...
Payloading adds the RTP header to the data packages, which encapsulates the payloaded data.
...
\verb+caps+ are used to describe metadata about the incoming video feed, and contains information such as encoding, protocol it is being sent via, framerate, and more.
...
udpsrc uri=rtp://172.25.26.10 port=5123 caps = "application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, sprop-parameter-sets=(string)\"Z01AFeygoP2AiAAAAwALuaygAHixbLA\\=\\,aOvssg\\=\\=\", payload=(int)96, ssrc=(uint)1171155755, clock-base=(uint)868988588, seqnum-base=(uint)65233"
\end{lstlisting}
