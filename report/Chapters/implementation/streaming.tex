...
This is necessary as the drone streams over TCP, and as described in Section \fxfatal{Ref til analysis om hvorfor no delay på streamen er vigtigt, og check at sætningen stadig giver mening} delay is not acceptable on the stream. \\
...
\subsection{Testing with a test-input source}%\fxfatal{Bedre overskrift}
...
Video data send via RTP must be pay-loaded, as a \ac{video frame} is larger than the maximum size allowed size of a UDP packet.
...
\verb+caps+ are used to describe metadata about the incoming video stream and contains information such as encoding, the protocol it is being sent using, \ac{framerate}, etc.
