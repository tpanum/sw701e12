...
Knowing that \deno{S} have a dynamic location relative to \deno{M}, this requires \deno{S} to send a signal to \{D} in order for \{M} to get the location of \{S} on the network.
...
The video displayed by \{B} is a stream, this requires that \deno{S} sends out a video stream.
