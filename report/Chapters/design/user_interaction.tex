...
\at{1} and \at{2} require that the user must be granted access to the system before being able to interact with it.
...
The user enters his login credentials, as described in Section~\ref{subsec:objects}, in the login view and \deno{B} sends a login request to \deno{M}.
...
The user's privileges are retrieved through the user model.
For each drone in the system it is checked if the user has privileges that grants him access to the drone.
From the drones list the user can edit a drone, or unlink a drone.
Drones are added automatically to the system through the initialization messages sent by \deno{S}.
To make a drone usable in \projectname{}, it must be added to a company as reflected in the object model, as described Section~\ref{subsec:objects}. \\
...
% As mentioned a user deletes a drone through the drones list sent to \deno{B} by \deno{M}.
% When a delete request is sent from \deno{B} to \deno{M} for a drone, \deno{M} deletes all links the drone has with other object in \deno{DB}.
...
% Some of the views require asynchronous behaviour, that allows the views to be updated without recomputing it on \deno{M}.
% %As an example when editing the current view can be updated asynchronously instead of having \deno{M} recompute it.
% In this way some of the computation is moved from \deno{M} to \deno{B}, as described in Section~\ref{sec:design_client}.
% %\fxfatal{User eller \deno{B} når vi snakker om brugeren - her er tale om browseren. /a}
