...
We know that operations must be sent to \deno{D} as UDP packages, and that \deno{S} must send as many as 30 packages per second. 
We also know, that if empty packages are sent, \deno{D} will return to its default hovering position. 
