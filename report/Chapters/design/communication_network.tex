...
\deno{M} verifies the identity of \deno{S} by \deno{S} sending its own unique identifier.
This unique identifier is a string that is hard coded into \deno{S} and unique for every \deno{S}.
It is generated before a new \deno{S} is shipped and ensured that it does not match any existing identifiers. 
