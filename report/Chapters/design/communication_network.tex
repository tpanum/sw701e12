...
If a user tries to interact with a drone the system behaves differently. A session key will be made on a \deno{S}, this key will be send to \deno{M} and giving to \deno{B}. With this session key it is possible for the user to communicate directly with \deno{S} without \deno{M}.
