...
\end{figure}
%\fxfatal{Skal der vÊre noget der indikerer kommunimation til og fra daemon og web pÂ figuren. - nej, denne figur viser jo bare intern kommunikation imellem de tre ting... /a}
...
Each controller in \projectname{} with a corresponding model, i.e. Affiliate Privilege, Company, Drone, Privilege, Role, and User, will have Create, Read, Update, and Delete (CRUD) \citep{CRUD} as default actions. 
...
One will handle the session communication with \deno{S}, as described in Section~\ref{sec:communication_network}.
The other handles receiving initialization messages from \deno{S}.
...
Session request are entered in a table in \deno{DB} by \deno{M} when a request for one is received from \deno{B}, as illustrated in Figure~\ref{fig:sessionkey_communication_b}.
...
If \deno{D} receives a valid session key from \deno{S}, it saves this in \deno{DB} and \deno{W} sends it to the user.
...
Each \deno{S} is associated with a \deno{drone} in the object model described in section~\ref{section:uml_notation}.
When \deno{D} receive an initialization message from \deno{S} it will create a \deno{drone} object and store it in the \deno{DB}.
In the \deno{drone} object the \verb+ip+,\verb+location+, and \verb+name+ will be retrieved from the initialization message send from \deno{S}.
The \verb+location+ is retrived from the IP and the \verb+name+ is the drones' serial code.
If \deno{M} receives an initialization message from a \deno{S} with a changed IP, it will update the \verb+ip+ and \verb+location+ of the \deno{S} in \deno{DB}.
The drones' serial code will be described in Section~\ref{sec:design_slave}.
\\

All communication between \deno{M} and \deno{S} is done using JavaScript Object Notation (\textit{JSON})\citep{JSON}.
The choice of JSON is described in Secion~\ref{sec:tools}.

\deno{D} will handle the session requests as a First In, First Out (FIFO) queue, to reduce the average time it takes to receive a session key after requestion one.
